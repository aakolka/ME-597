...
Task: 6
Goal: Track a red ball in Gazebo and use PID to follow and center the red ball
"""
...
class RedBallTracker(Node):
    ...
    def move(self, contour_area):
        """
        Adjusts robot linear velocity to maintain target distance from the ball.
        Args:
            - contour_area : Area of the detected ball's contour (proxy for distance)
        Returns:
            - None
        """
        target = 32250 # Desired pixel area 
    ...
    def rotate(self,cx):
        """
        Adjusts robot angular velocity to center the ball horizontally.
        Args:
            cx : x-coordinate of the ball's centroid in the image
        Returns:
            None
        """
    ...
    def callback(self,ros2_img):
        """
        Processes each incoming camera image to detect the red ball, draw a bounding box,
        and control the robot to follow it.
        Args:
            ros2_img : ROS 2 Image message from camera
        Returns:
            None
        """
